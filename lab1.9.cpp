...
int main(){
    ...
    while (1){
        ...
        //задаем размер стека (2Мбайт, 1Мбайт, 512 Кбайт)
        pthread_attr_t thread_attr;
        // Устанавливаем размер стека
	err = pthread_attr_setstacksize(&thread_attr, 2*1024*1024);
	if(err != 0) {
	...
	exit(-1);
        ...
        // Создаём поток
        err = pthread_create(&thread, &thread_attr, request_handle, (void*)client);
        if(err != 0){
